...
- **Mutação**: Pitest nos três módulos, com limite de **90% de mutantes mortos** em `:domain` e
  `:server` e **50%** no `:app` (ADR 0016, que lista as exclusões e o motivo de cada uma). Fora do
  `check`: roda toda semana no CI e à mão, e a força dos testes aparece num badge do README (ADR 0023).
